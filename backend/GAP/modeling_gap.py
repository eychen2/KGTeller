...
#         adj_matrix_emb = adj_matrix_emb.contiguous().view(-1, node_len * node_len, emb_dim).transpose(0, 1)  # (node_len * node_len) * batch * emb_dim
        # adj_matrix = adj_matrix.contiguous().view(-1,node_len*node_len)
...
class SelfStructureAttention(nn.Module):
    ...
    def __init__(
        ...
        self.v_proj = nn.Linear(embed_dim, embed_dim, bias=bias)
        self.q_proj = nn.Linear(embed_dim, embed_dim, bias=bias)
        # self.t_enc_proj = nn.Linear(51,51,bias=bias)
        self.t_emb_lookup = nn.Embedding(3,1)
...
#         """Input shape: Time(SeqLen) x Batch x Channel"""
        ...
        #OLD
        # T = torch.zeros_like(attn_mask, dtype=torch.float)
        # attn_mask_bool = (attn_mask==2)
        # T.masked_fill_(attn_mask_bool,  -1.0)
        # T = self.t_enc_proj(T)
        
        #NEW
        T = torch.zeros_like(attn_mask, dtype=torch.long)
        attn_mask_bool = (attn_mask==60)
        T.masked_fill_(attn_mask_bool,  0)
        attn_mask_bool = (attn_mask==2)
        T.masked_fill_(attn_mask_bool,  2)
        attn_mask_bool = (attn_mask==1)
        T.masked_fill_(attn_mask_bool,  1)
...
#             new_attn_mask.masked_fill_(attn_mask_bool,  -10000.0)
        ...
        assert key_padding_mask is None or key_padding_mask.size()[:2] == (bsz, src_len,)
#         if key_padding_mask is not None:  # don't attend to padding symbols
#             key_padding_mask_squared_minus_1 = key_padding_mask.unsqueeze(1).repeat(1,tgt_len-1,1).permute(0,2,1)
#             mask_box = torch.zeros((bsz,tgt_len,src_len), dtype=torch.bool).cuda()
#             mask_box[:,:,1:] = key_padding_mask_squared_minus_1
#             default_1_column = torch.logical_or(key_padding_mask.unsqueeze(1),mask_box)
#             default_1_column = default_1_column.unsqueeze(1).repeat(1,self.num_heads,1,1).view(bsz*self.num_heads,tgt_len,src_len)
            # attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len)
            #reshaped = key_padding_mask.unsqueeze(1).unsqueeze(2)
            # attn_weights = attn_weights.masked_fill(default_1_column, float("-inf"))
            # attn_weights = attn_weights.view(bsz * self.num_heads, tgt_len, src_len)
        ...
        attn_output = attn_output.transpose(0, 1).contiguous().view(tgt_len, bsz, embed_dim)
        attn_output = self.out_proj(attn_output)
        #attn_output, attn_weights = self.mh_attn(query,key,key,key_padding_mask=key_padding_mask,attn_mask=default_1_column)
...
#         x_node = scatter_mean(x_ori, input_node_ids, dim=1)  # batch * node_size * embed_dim
#         # print(x_ori.shape)
#         #print(input_node_ids[0,:])
#         # print(x_node.shape)
#         # print(node_attention_mask.shape)
...
#             # print(dfsdfsdfsdfsdfsdfs)
#             attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len)
#             reshaped = key_padding_mask.unsqueeze(1).unsqueeze(2)
#             attn_weights = attn_weights.masked_fill(reshaped, float("-inf"))
#             attn_weights = attn_weights.view(bsz * self.num_heads, tgt_len, src_len)
#             # print(reshaped[0,:,:,:])
#             #for those values which have node there is no negative infinity, instead it is same small value
#             #has same value (non -infs in last rows)
#             print(attn_weights[0,:,0])
#             print(key_padding_mask[0,:])
            
#             #has all non-infs for one and same value but non -infs in last rows
#             # print(attn_weights[0,:,5])
#             # print(key_padding_mask[0,:])
            
#             # -inf everywhere
#             # print(attn_weights[0,:,35])
#             print(dfadfda)
#         attn_weights = F.softmax(attn_weights, dim=-1)
#         attn_probs = F.dropout(attn_weights, p=self.dropout, training=self.training,)
